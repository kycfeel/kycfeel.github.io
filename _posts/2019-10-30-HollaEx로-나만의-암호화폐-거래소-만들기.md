...
HollaEx는 필자가 몸담아 일하고 있는 [bitHolla](https://bitholla.com) 에서 만든 암호화폐 거래소 솔루션이다. 뭐, 기술적으로는 암호화폐 거래소 뿐만 아니라 거래를 필요로 하는 모든 분야에 사용할 수 있지만, 어쨋든 그렇다. HollaEx를 사용하면 암호화폐 거래소를 만들 기술이나 기반이 부족하더라도, 약간의 노력만 기울이면 나만의 거래소를 뚝딱 만들 수 있다는 말씀. HollaEx 자체는 1~2년 전에도 존재했고, [EXIR](https://www.exir.io) 등 진작부터 HollaEx를 사용해 만들어진 실제 거래소도 있지만, 이번 리뉴얼을 거치며 그 접근성을 한껏 끌어올렸다. 
...
![hollaex_kit_main](../_images/hollaex_kit_main.png)
...
![vault_key_generation](../_images/vault_key_generation.png)
...
![hollaex_setup_confirmation](../_images/hollaex_setup_confirmation.png)
...
![hollaex_setup_add_new_coin_and_pair](../_images/hollaex_setup_add_new_coin_pair.png)
...
![add_coin_confirmation](../_images/add_coin_confirmation.png)

모든 정보가 올바른지 다시한번 확인하고, `Y`를 선택하면 아래와 같이 내 거래소에 자동으로 새 코인이 추가된다.

![add_coin_process](../_images/add_coin_process.png)

정상적으로 추가되었다면 HollaEx CLI가 아래와 같이 멋들어진 그래픽으로 친절히 알려준다. 

![add_coin_eht_success](../_images/add_coin_eth_success.png)
...
![add_pair_form](../_images/add_pair_form.png)
...
![add_pair_success](../_images/add_pair_success.png)
...
![hollaex_start](../_images/hollaex_start.png)
...
![hollaex_health_localhost](../_images/hollaex_health_localhost.png)

`http://<내_도메인>/v1/constant` 주소로 접근하면 지금 내 거래소에 등록된 코인과 거래 페어 리스트를 볼 수 있다.

![hollaex_constant_localhost](../_images/hollaex_constant_localhost.png)
...
> ![dockerd_memory_allocation_configure](../_images/dockerd_memory_allocation_configure.png)


빌드가 끝나고 정상적으로 웹이 실행된다면, 아래와 같은 안내 메시지가 뜰 것이다.

![hollaex_web_success](../_images/hollaex_web_success.png)
...
![hollaex_signin](../_images/web_client_login.png)

짠. 정상적으로 로그인이 되었다면 아래와 같은 메인 페이지가 독자를 반겨줄 것이다. 

![web_successful_login](../_images/web_login_success.png)
