...
이제 같은 폴더 안의 `head.html`에서 호출만 하면 된다. Jekyll은 표준 [Liquid](https://github.com/Shopify/liquid) 문법을 모두 지원원하니 `head.html` 파일의 `<head>` 태그 안 어디나 <code>{% include google_analytics.html %}</code> 블록을 집어넣어주면 작업 끝.
